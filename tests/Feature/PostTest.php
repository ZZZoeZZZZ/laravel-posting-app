...
class PostTest extends TestCase
{
    ...
    // 未ログイン時には投稿一覧ページにアクセスできない
    public function test_guest_cannnot_access_posts_index()
    {
        $response = $this->get(route('posts.index'));
        $response->assertRedirect(route('login'));
    }

    // ログイン済みは投稿一覧ページにアクセスできる
    public function test_user_can_access_posts_index()
    {
        $user = User::factory()->create();
        $post = Post::factory()->create(['user_id' => $user->id]);

        // actingAs：作成したユーザーでログインする
    ...
    public function test_guest_cannnot_access_posts_edit()
    {
        $user = User::factory()->create();
        $post = Post::factory()->create(['user_id' => $user->id]);
    ...
    // ログイン済みのユーザーは他人の投稿の編集ページにアクセスできない
    ...
    // ログイン済みのユーザーは自分の投稿の編集ページにアクセスできる
    ...
    public function test_guest_cannnot_update_post()
    {
        $user = User::factory()->create();
    ...
    public function test_guest_cannnot_destroy_post()
    {
        $user = User::factory()->create();
        $post = Post::factory()->create(['user_id' => $user->id]);
